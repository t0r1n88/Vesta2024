...
if __name__ == '__main__':
    window = Tk()
    window.title('Веста Обработка таблиц и создание документов ver 1.63')
    # Устанавливаем размер и положение окна
    ...
    lbl_hello = Label(create_batch_frame_description,
                      text='Пакетная генерация документов по шаблонам\n'
                           'ПРИМЕЧАНИЯ\n'
                           'В файле с данными должно быть не менее 2 листов !!!\n'
                           'На первом листе константы (данные одинаковые для всех создаваемых документов),\n на втором листе данные которые изменяются (ФИО, реквизиты и т.п.).\n'
                           'Заголовок таблицы должен занимать только первую строку!\n'
                           'Для корректной работы программы уберите из таблицы\nобъединенные ячейки'
                      , width=60)
    lbl_hello.pack(side=LEFT, anchor=N, ipadx=25, ipady=10)
    ...
    label_name_column_batch = Label(create_batch_frame_action,
                                    text='3) Введите название колонки на втором листе в таблице\n по которой будут создаваться имена файлов\n'
                                         )
