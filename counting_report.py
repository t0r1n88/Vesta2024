...
def check_range_one_file(df:pd.DataFrame):
    """
    Функция для проверки корректности записи диапазона в случае одного файла
    :param датафрейм: параметры
    :return: Правильно или Неправильно
    """
    error_df = pd.DataFrame(
    ...
    for row in df.itertuples():
        ...
        result = re.search(pattern,name_range)
        if result:
            out_dct[f'{name_range}'] = {'Диапазон':name_range,'Количество колонок':number_cols}

        else:
            temp_error_df = pd.DataFrame(columns=['Название файла','Название листа', 'Описание ошибки'],
                data=[['Ошибка в файле параметров', f'', f'Ошибка в написании диапазона- {name_range} . Правильный формат написания 1,2,3 буквы латинского алфавита потом число потом двоеточие потом одна или 1,2,3 буквы латинского алфавита потом число. Например F2:AZ10,XFD5:XFD7']])
            error_df = pd.concat([error_df,temp_error_df])
    return out_dct,error_df
...
def counting_table_report(file_params:str, report_dir:str,path_end_folder:str):
    ...
    try:
        ...
        # подсчитываем количество файлов Excel
        ...
        if len(lst_xlsx) == 1:
            try:
                ...
                messagebox.showerror('Веста Обработка таблиц и создание документов',
                                     'Не удалось обработать файл с параметрами обработки отчетов!\n'
                                     'Проверьте файл на повреждения. Пересохраните в новом файле.')
            params_df.dropna(thresh=2, inplace=True)  # очищаем от неполных строк
            ...
            dct_params, error_params_df = check_range_one_file(params_df)  # создаем словарь с параметрами и датафрейм с ошибками
            ...
            for key, value in dct_work.items():
                ...
                dct_work[key]['Список для проверки'] = pd.DataFrame(
                    columns=[f'Колонка {i}' for i in range(1, value['Количество колонок'] + 1)])
                dct_work[key]['Список для проверки'].insert(0, 'Номер строки', '')
                dct_work[key]['Список для проверки'].insert(1, 'Название листа', '')

            # обрабатываем как единичный файл
            for dirpath, dirnames, filenames in os.walk(report_dir):
                for file in filenames:
                    ...
                    if not file.startswith('~$') and (file.endswith('.xlsx') or file.endswith('.xlsm')):
                            ...
                            for name_sheet in wb.sheetnames:
                                try:
                                    ...
                                    temp_error_df = pd.DataFrame(
                                        data=[[f'{name_file}',
                                               f'{name_sheet}',
                                               f'Указанный диапазон {value["Диапазон"]} превышает допустимые для листа xlsx значения. Допустимые значения от 1 до 1048576'
                                               ]],
                                ...
                                if len(data[0]) != value['Количество колонок']:
                                    temp_error_df = pd.DataFrame(
                                        data=[[f'{name_file}',
                                               f'{name_sheet}',
                                               f'Указанный диапазон {value["Диапазон"]} не совпадает с количество обрабатываемых колонок. В параметрах отчета указано количество обрабатываемых колонок равное {value["Количество колонок"]}, а указанный диапазон занимает {len(data[0])}'
                                               ]],
                                        columns=['Название файла', 'Название листа',
                                                 'Описание ошибки'])
                                    error_df = pd.concat([error_df, temp_error_df], axis=0,
                                ...
                                check_lst.insert(1, 'Название листа')
                        ...
                        for key, value in dct_work.items():
                            value['Данные'].columns = [f'Колонка {i}' for i in
                                                       range(1, value['Данные'].shape[1] + 1)]
                            value['Данные'].index = [f'Строка {i}' for i in
                                                     range(1, value['Данные'].shape[0] + 1)]
            ...
            # обрабатываем множественный файл
            for dirpath, dirnames, filenames in os.walk(report_dir):
                for file in filenames:
                    if file.endswith('.xls') or file.endswith('.ods'):
                        temp_error_df = pd.DataFrame(
        ...
        messagebox.showerror('Веста Обработка таблиц и создание документов',
                             f'Не найдено ни одного корректного диапазона ячеек. Адрес ячейки должен выглядить так A5:C15, H20:H155 и т.п.')
    except NotFile:
        messagebox.showerror('Веста Обработка таблиц и создание документов',
                             f'Не найдено ни одного файла Excel (xlsx).')
    else:
