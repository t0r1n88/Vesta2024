...
class NotColumn(Exception):
    """
    Исключения для обработки варианта когда отсутствует указанная колонка
    """
    pass

class DeclNameDf(Exception):
    """
    Исключение для обработки когда в датафрейме уже есть ранее созданные колонки с помощью склонения падежей
    """
    pass
...
def declension_fio_by_case(fio_column,data_decl_case,path_to_end_folder_decl_case):
    ...
    try:
        ...
        # проверяем наличие колонок с падежами в файле
        set_new_columns = {'Родительный_падеж','Дательный_падеж','Винительный_падеж','Творительный_падеж','Предложный_падеж',
                           'Фамилия_инициалы','Инициалы_фамилия','Фамилия_инициалы_пробел','Инициалы_фамилия_пробел',
                           'Фамилия_инициалы_род_падеж','Фамилия_инициалы_род_падеж_пробел','Инициалы_фамилия_род_падеж','Инициалы_фамилия_род_падеж_пробел',
                           'Фамилия_инициалы_дат_падеж','Фамилия_инициалы_дат_падеж_пробел','Инициалы_фамилия_дат_падеж','Инициалы_фамилия_дат_падеж_пробел',
                           'Фамилия_инициалы_вин_падеж','Фамилия_инициалы_вин_падеж_пробел','Инициалы_фамилия_вин_падеж','Инициалы_фамилия_вин_падеж_пробел',
                           'Фамилия_инициалы_твор_падеж','Фамилия_инициалы_твор_падеж_пробел','Инициалы_фамилия_твор_падеж','Инициалы_фамилия_твор_падеж_пробел',
                           'Фамилия_инициалы_пред_падеж','Фамилия_инициалы_пред_падеж_пробел','Инициалы_фамилия_пред_падеж','Инициалы_фамилия_пред_падеж_пробел'}

        check_cols = set_new_columns.intersection(set(df.columns))
        if len(check_cols) != 0:
            raise DeclNameDf
        ...
        messagebox.showerror('Веста Обработка таблиц и создание документов',
                             f'В таблице не найдена указанная колонка c ФИО: {fio_column}')
    except DeclNameDf:
        messagebox.showerror('Веста Обработка таблиц и создание документов',
                             f'В таблице присутствуют колонки созданные с помощью функции Склонение ФИО по падежам.\n'
                             f'Удалите в файле с данными колонки: {check_cols}')
        logging.exception('AN ERROR HAS OCCURRED')
